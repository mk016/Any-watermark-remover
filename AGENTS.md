...
- Preferred check:
  1. Open the Tampermonkey editor for `Gemini NanoBanana image watermark removal`
...
- If the original URL binding is unavailable when the required download/original request arrives, the action must fail closed with:
  - `Unable to retrieve the original image，please refresh the page and try again`
...
- When the userscript intercepts `download full-size image` at capture time and calls `preventDefault()/stopImmediatePropagation()`, Gemini's own download flow is blocked before it can issue its native `c8o8Fe` / `rd-gg-dl` chain.
...
- `download full-size image` produced a browser `download` event
...
- no `Unable to retrieve the original image，please refresh the page and try again` alert appeared
- `copy image` wrote an `image/png` item to the clipboard without a failure alert
