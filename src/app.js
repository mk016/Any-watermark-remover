...
const TEXT = {
    ...
    unsupported: 'Browser does not support image copy',
    copied: 'Copied!',
    copy: 'Copy result',
    copyFailed: 'Copy failed',
    unsupportedFile: 'Please select JPG, PNG, or WebP images, or MP4/WebM/MOV videos.',
    fileTooLarge: 'The image debug entry currently skips files larger than 20MB. Videos will go to the video debug page.',
...
async function routeVideoFile(file) {
    try {
        ...
        hideLoading();
        console.error(error);
        setStatusMessage(error.message || 'Unable to enter the video debug flow; please open the video page and reselect the file.', 'warn');
    }
}
...
async function consumePendingImageHandoff() {
    ...
    try {
        ...
        setStatusMessage(error.message || 'Unable to cache image read; please reselect a file.', 'warn');
    }
}
