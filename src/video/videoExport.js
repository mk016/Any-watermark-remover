...
function createRuntimeCanvas(width, height) {
    ...
    throw new Error('No available Canvas in the current environment');
}
...
async function getVideoContext(file) {
    ...
    if (!videoTrack) {
        input.dispose();
        throw new Error('No processable video track found in the file');
    }
...
export async function runVideoRemovalExport(file, detected, options = {}) {
    ...
    if (!detection.isConfident && options.allowLowConfidence !== true) {
        throw new Error('Video watermark detection confidence is too low; export stopped. Try again with low-confidence export enabled.');
    }
    ...
    if (!canEncodeAvc) {
        throw new Error('Current browser does not support WebCodecs H.264/AVC encoding; please use a newer Chrome or Edge.');
    }
